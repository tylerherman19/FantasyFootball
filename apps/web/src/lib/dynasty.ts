 ...
 * from the market, ages from birthdates, production from three seasons of real
...
export interface DynastyAsset {
  ...
  /**
   * Market value per point of current production.
   *
   * High means the market is paying for something other than this year — youth,
   * name, draft capital. That is exactly what a contending roster should be
...
export const buildDynastyView = async (
  ...
  const valueByAge = bands.map(({ band, min, max }) => {
    ...
    return {
      band,
      value: inBand.reduce((sum, asset) => sum + asset.marketValue, 0),
      count: inBand.length,
    };
  });
  ...
  const valueByPosition = positions
    .map((position) => {
      ...
      return {
        position,
        value: group.reduce((sum, asset) => sum + asset.marketValue, 0),
        count: group.length,
      };
    })
  ...
  const leagueMedianValue = median(profiles.map((entry) => entry.marketValue));
  ...
  const sellHigh = [...valued]
    .filter((asset) => asset.windowYears !== null && asset.windowYears < 1.5 && asset.marketValue > 0)
